...
/**
 * This class is designed to test each step of the huffman coding process when
 * run
 * 
...
public class Driver {
    public static void main(String[] args) {
        String[] methods = { "makeSortedList", "makeTree", "makeEncodings", "encode", "decode" },
                options = { "Test new file", "Test new method on the same file", "Quit" };
        int repeatChoice = 0;
    ...
    private static void testSortedList(String input) {
        ...
        System.out.println("Note that the decimals are rounded to 2 decimal places.\n");
        System.out.printf("%s->%.2f", charToString(sortedList.get(0).getCharacter()), sortedList.get(0).getProbOcc());
        for (int i = 1; i < sortedList.size(); i++) {
            char c = sortedList.get(i).getCharacter();
            System.out.printf(", %s->%.2f", charToString(c), sortedList.get(i).getProbOcc());
        }
        System.out.println();
    }
    ...
    private static void printTree(TreeNode n, String indent, boolean isRight, boolean isRoot) {
        ...
        // If we have an associated character print it too, otherwise just the
        // probability
        if (n.getData().getCharacter() != null)
            System.out.print(charToString(n.getData().getCharacter()) + "->");
        System.out.printf("%.2f ", n.getData().getProbOcc());
        System.out.println();
    ...
    private static void testMakeEncodings(String input) {
        ...
        for (int i = 0; i < 128; i++)
            if (encodings[i] != null) {
                if (!first)
                    System.out.print(", ");
                System.out.printf("%s->%s", charToString((char) i), encodings[i]);
                first = false;
            }
        System.out.println();
    }
